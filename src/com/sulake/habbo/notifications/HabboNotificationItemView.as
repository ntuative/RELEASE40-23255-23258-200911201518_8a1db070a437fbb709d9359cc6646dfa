package com.sulake.habbo.notifications
{
   ...
   public class HabboNotificationItemView implements ICoreUpdateReceiver
   {
      ...
      public static const const_720:int = 70;
      
      private static const const_751:int = 2;
      ...
      public function HabboNotificationItemView(param1:IAsset, param2:IHabboWindowManager, param3:Map, param4:Map, param5:IHabboToolbar)
      {
         ...
         if(_loc7_ != null)
         {
            var_1177 = 0 - _loc7_.rectangle.bottom;
         }
         else
         {
            var_1177 = 15;
         }
         var_1419 = _window.height;
         var_546 = 4;
         var_392 = 0;
         _state = const_336;
         var_195 = param5;
      ...
      private function setNotificationText(param1:String) : void
      {
         ...
         _window.height = 0;
         _loc2_.text = param1;
         _loc2_.height = _loc2_.textHeight + var_1177;
         if(_window.height < var_1419)
         {
            _window.height = var_1419;
         }
      }
      ...
      public function update(param1:uint) : void
      {
         ...
         switch(_state)
         {
            ...
            case const_376:
               var_784 += param1;
               _loc2_ = Number(var_784) / Number(var_391["time_fade_in"]);
               if(var_784 > int(var_391["time_fade_in"]))
               {
                  startDisplay();
               }
               adjustBlend(_loc2_);
            ...
            case const_751:
               var_962 += param1;
               if(var_962 > int(var_391["time_display"]) && !var_1178)
               {
                  startFadeOut();
               }
               break;
            case const_752:
               var_785 += param1;
               _loc3_ = 1 - Number(var_785) / Number(var_391["time_fade_out"]);
               adjustBlend(_loc3_);
               if(var_785 > int(var_391["time_fade_in"]))
               {
                  startIdling();
               }
         }
      ...
      public function reposition(param1:int = -1) : void
      {
         ...
         var _loc3_:int = var_195.orientation == "TOP" ? int(const_335) : 0;
         var _loc4_:int = var_195.orientation == "BOTTOM" ? int(_loc2_.height - const_335) : int(_loc2_.height);
         var _loc5_:int = var_195.orientation == "LEFT" ? int(const_335) : 0;
         var _loc6_:int = var_195.orientation == "RIGHT" ? int(_loc2_.width - const_335) : int(_loc2_.width);
         if(false)
         {
            if(false)
            {
               _window.x = _loc5_ + const_370;
            }
            else
            {
               _window.x = _loc6_ - 0 - const_370;
            }
         }
         else if(false)
         {
         ...
         if(false)
         {
            if(false)
            {
               _window.y = _loc3_ + var_546;
            }
            else
            {
               _window.y = _loc4_ - 0 - var_546;
            }
         }
         else if(false)
         {
